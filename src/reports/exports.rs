...
pub(crate) async fn export(db: &dyn ReportDB, cfg: &ExportConfig) -> Result<()> {
...
async fn simple_export(reports: &Vec<PlayerReport>, cfg: &ExportConfig) -> Result<()> {
    let content = reports.iter().filter(|r|r.verified).map(|r|r.steamid.steam64().to_string()).join("\n");
    fs::write(&cfg.id_list_filename, content.as_bytes()).await?;
    Ok(())
}
...
async fn tf2bd_export(reports: &Vec<PlayerReport>, cfg: &ExportConfig) -> Result<()> {
    ...
    let players = reports.iter().filter(|r|r.verified).into_group_map_by(|r|r.steamid).into_iter().map(|(id, reps)|{
        ...
        let attrs = reps.iter().map(|r|r.attribute.to_string()).collect::<Vec<_>>();
        let proof = reps.iter().map(|r|&r.thread_url).collect::<Vec<_>>();
        let last_seen = reps.iter().map(|r|r.last_seen).max().unwrap();
    ...
    let description = "List of cheaters reported in the hackerpolice channel on the Vorobey discord server, last updated {now}";
